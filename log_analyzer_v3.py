...
class LogicChecker:
    ...
    PATTERNS = {
        ...
        "b_data_lost": re.compile(
            r"\[B[12]\].*(?:数据丢失|data.*lost|None.*price|price.*None|"
            r"NaN|inf|空数据|no data|数据为空|bars不足|K线不足)",
            re.I
        ),
        "b_hang_timeout": re.compile(
            r"\[B[12]\].*(?:Timeout|超时|卡死|hang|deadlock|"
    ...
    def check(self, log_lines: List[str], detector: IssueDetector) -> Dict:
        ...
        for line in log_lines:
            ...
            for _bk, _br in (("b1_code_error", "B1"), ("b2_code_error", "B2")):
            ...
            # 维度1d: 通用兜底
            if not _b_matched:
                for _bk, _br in (("b1_error", "B1"), ("b2_error", "B2")):
                    if self.PATTERNS[_bk].search(line):
                        results[f"{_br.lower()}_errors"] += 1
                        ts_m = re.search(r'(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2})', line)
                        results["b_error_details"].append({
...
class ReportGenerator:
    ...
    def generate_daily_report(self, date: str, detector: IssueDetector,
        ...
        if _b1e + _b2e + _b1blk > 0:
            lines.extend([
                "",
                "### B1/B2 通道错误",
                "",
                "**按通道**:",
                f"- B1(主决策+BTC短线): {_b1e}次" + (f" + {_b1blk}次非B1拦截" if _b1blk else ""),
                f"- B2(TSLA期权): {_b2e}次",
            ])
            # 维度1: 通道本身错误
            lines.extend([
                ...
                f"- 🔴 代码BUG (语法/类型/属性): B1={_b_code.get('B1',0)} B2={_b_code.get('B2',0)}",
