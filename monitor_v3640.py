...
def print_position_control_status(state):
    ...
    # 表头
    print(f"  {pad_to_width('SYMBOL', WS)} {pad_to_width('POS', WP)} {pad_to_width('MODE', WM)}  {pad_to_width('BUY', WB)}  {pad_to_width('SELL', WL)}")
    print(f"  {'-'*WS} {'-'*WP} {'-'*WM}  {'-'*WB}  {'-'*WL}")
    ...
    for symbol, sym_data in state.items():
        if isinstance(sym_data, dict) and "position_units" in sym_data:
            ...
            # 判断Position Control模式和允许的操作 (文字和颜色分开)
            if position <= 0:
                mode = "空仓"
                buy_text, buy_color = "自由", C_G
                sell_text, sell_color = "禁止", C_R
            elif position == 1:
                mode = "仓位1(单向卖)"
                buy_text, buy_color = "自由", C_G
                if big_trend == "DOWN" or current_trend == "DOWN":
                    sell_text, sell_color = "可清仓✓", C_G
                else:
                    sell_text, sell_color = "需D", C_Y
            elif position == 2:
                mode = "仓位2"
                buy_text, buy_color = "自由", C_G
                sell_text, sell_color = "EMA10", C_C
            elif position == 3:
                mode = "仓位3(EMA10)"
                buy_text, buy_color = "EMA10", C_C
                sell_text, sell_color = "EMA10", C_C
            elif position == 4:
                mode = "仓位4(共振买)"
                if big_trend == "UP" and current_trend == "UP":
                    buy_text, buy_color = "共振✓", C_G
                else:
                    buy_text, buy_color = "需U+U", C_Y
                sell_text, sell_color = "自由", C_G
            else:  # position >= 5
                mode = "满仓"
                buy_text, buy_color = "禁止", C_R
                sell_text, sell_color = "自由", C_G

            pos_str = f"{position}/{max_units}"
            ...
            print(f"  {pad_to_width(symbol, WS)} {_c(pos_str, WP, pos_col)} {pad_to_width(mode, WM)}  {_c(buy_text, WB, buy_color)}  {_c(sell_text, WL, sell_color)}")
...
# v3.653: OrderFlow 监控面板
ORDERFLOW_STATE_FILE = "state/orderflow_state.json"

def print_orderflow_panel():
    """v3.653: 打印OrderFlow量价监控面板 (Phase 1 观察模式)"""
    try:
        if not os.path.exists(ORDERFLOW_STATE_FILE):
            return  # 无数据时静默跳过

        with open(ORDERFLOW_STATE_FILE, 'r', encoding='utf-8') as f:
            of_state = json.load(f)
        ...
        total_width = 90
        print("\n" + "=" * total_width)
        print(f" OrderFlow 监控 (Phase 1 观察) | 数据源: {ORDERFLOW_STATE_FILE}")
        print("-" * total_width)

        # 表头
        header = f" {'品种':<10}{'RelVol':>8}{'CVD趋势':>10}{'SQS':>8}{'VF结果':>10}{'更新时间':>18}"
        print(header)
        print("-" * total_width)

        # VF颜色映射
        vf_colors = {"UPGRADE": C_GB, "PASS": C_G, "DOWNGRADE": C_Y, "REJECT": C_R}

        for symbol in sorted(of_state.keys()):
            data = of_state[symbol]
            ...
            # 截取时间部分
            if len(updated) > 10:
                updated = updated[11:]  # HH:MM:SS

            # RelVol颜色
            if rel_vol >= 1.5:
                rv_str = C_GB + f"{rel_vol:>7.2f}" + C_0
            elif rel_vol < 0.5:
                rv_str = C_R + f"{rel_vol:>7.2f}" + C_0
            else:
                rv_str = f"{rel_vol:>7.2f}"

            # SQS颜色
            if sqs >= 0.7:
                sqs_str = C_G + f"{sqs:>7.2f}" + C_0
            elif sqs < 0.3:
                sqs_str = C_R + f"{sqs:>7.2f}" + C_0
            else:
                sqs_str = f"{sqs:>7.2f}"

            # VF颜色
            vf_color = vf_colors.get(vf, "")
            vf_str = vf_color + f"{vf:>9}" + C_0 if vf_color else f"{vf:>9}"

            print(f" {symbol:<10}{rv_str}{cvd_trend:>10}{sqs_str}{vf_str}{updated:>18}")
...
def print_system_improvements_panel():
    """v3.654: 系统改善观察面板 (Phase 1 统一监控)"""
    try:
        total_width = 90
        items = []

        # 1. 策略评估指标 (SYS-018)
        metrics_file = "state/strategy_metrics.json"
        if os.path.exists(metrics_file):
            ...
            items.append(f" Strategy | Score:{score:.0f}/100 {grade} | "
                         f"Sharpe:{m.get('sharpe', 0):.2f} WR:{m.get('win_rate', 0):.1%} "
                         f"PF:{m.get('profit_factor', 0):.2f} MaxDD:{m.get('max_drawdown', 0):.1%}")

        # 2. 组合熔断状态 (SYS-017) — 从主程序HTTP读取
        breaker_file = "state/portfolio_breaker.json"
        if os.path.exists(breaker_file):
            with open(breaker_file, 'r', encoding='utf-8') as f:
                bk = json.load(f)
            items.append(f" Breaker  | 连亏:{bk.get('consecutive_losses', 0)} "
                         f"日亏:{bk.get('daily_pnl_pct', 0):.1f}% "
                         f"T1:{bk.get('tier1_active', False)} T2:{bk.get('tier2_active', False)} "
                         f"Halt:{bk.get('daily_halt', False)}")

        # 3. 相关性风控 (SYS-019)
        corr_file = "state/correlation_state.json"
        if os.path.exists(corr_file):
            with open(corr_file, 'r', encoding='utf-8') as f:
                corr = json.load(f)
            risk = corr.get("risk_level", "?")
            risk_color = C_R if risk == "HIGH" else C_Y if risk == "MEDIUM" else C_G
            items.append(f" CorrRisk | {risk_color}{risk}{C_0} | "
                         f"高相关对:{len(corr.get('high_corr_pairs', {}))} "
                         f"集群:{len(corr.get('clusters', []))} "
                         f"集中度:{corr.get('max_concentration', 0):.0%}")

        # 4. DTW模式匹配 (SYS-013)
        dtw_file = "state/dtw_patterns.json"
        if os.path.exists(dtw_file):
            with open(dtw_file, 'r', encoding='utf-8') as f:
                dtw = json.load(f)
            pats = dtw.get("patterns", [])
            wins = sum(1 for p in pats if p.get("outcome") == "WIN")
            loses = sum(1 for p in pats if p.get("outcome") == "LOSE")
            total = wins + loses
            wr = wins / total if total > 0 else 0
            items.append(f" DTW     | 模式库:{len(pats)} | 胜:{wins} 负:{loses} WR:{wr:.0%}")

        # 5. OrderFlow量价增强 (SYS-025~028)
        of_file = "state/orderflow_state.json"
        if os.path.exists(of_file):
            with open(of_file, 'r', encoding='utf-8') as f:
                of_data = json.load(f)
            _th_syms = [s for s, d in of_data.items() if "trend_health" in d]
            _vp_syms = [s for s, d in of_data.items() if "vp_poc" in d]
            _dv_syms = [s for s, d in of_data.items() if "donchian_vol_score" in d]
            if _th_syms or _vp_syms or _dv_syms:
                _avg_th = sum(of_data[s]["trend_health"] for s in _th_syms) / len(_th_syms) if _th_syms else 0
                items.append(f" OFplus  | TrendHealth:{len(_th_syms)}品种 avg={_avg_th:.2f} | "
                             f"VP:{len(_vp_syms)}品种 | DonchVol:{len(_dv_syms)}品种")

        # 6. 实时Performance Dashboard (SYS-029: broker对账优先)
        _broker_pnl_file = "state/broker_pnl.json"
        _broker_shown = False
        if os.path.exists(_broker_pnl_file):
            try:
                with open(_broker_pnl_file, 'r', encoding='utf-8') as f:
                    _bp = json.load(f)
                _schwab_pnl = _bp.get("schwab", {}).get("total_realized", 0)
                _cb_pnl = _bp.get("coinbase", {}).get("total_realized", 0)
                _total_pnl = _bp.get("total_pnl", 0)
                _schwab_c = C_G if _schwab_pnl >= 0 else C_R
                _cb_c = C_G if _cb_pnl >= 0 else C_R
                _total_c = C_G if _total_pnl >= 0 else C_R
                _upd = _bp.get("updated_at", "")[:10]
                items.append(f" PnL     | Schwab:{_schwab_c}${_schwab_pnl:+,.0f}{C_0} "
                             f"CB:{_cb_c}${_cb_pnl:+,.0f}{C_0} "
                             f"Total:{_total_c}${_total_pnl:+,.0f}{C_0} ({_upd})")
                _broker_shown = True
            except Exception:
                pass
        if not _broker_shown:
            pnl_file = "plugin_profit_state.json"
            if os.path.exists(pnl_file):
                ...
                total_trades = 0
                wins_t = 0
                for sym, sym_data in pnl_data.items():
                    if isinstance(sym_data, dict):
                        closed = sym_data.get("closed_trades", [])
                        for t in closed:
                            pnl_val = t.get("pnl", 0)
                            total_pnl += pnl_val
                            total_trades += 1
                            if pnl_val > 0:
                                wins_t += 1
                if total_trades > 0:
                    wr_t = wins_t / total_trades
                    pnl_color = C_G if total_pnl > 0 else C_R
                    items.append(f" PnL     | {pnl_color}${total_pnl:+.2f}{C_0} | "
                                 f"交易:{total_trades} 胜率:{wr_t:.0%}")

        # 7. Anti-Whipsaw HOLD带 (SYS-030)
        gt_file = "global_trend_state.json"
        if os.path.exists(gt_file):
            with open(gt_file, 'r', encoding='utf-8') as f:
                gt = json.load(f)
            syms_data = gt.get("symbols", {})
            _hb_active = sum(1 for s, d in syms_data.items() if d.get("hold_band_active"))
            _hb_total = len(syms_data)
            _hb_mids = [(s, d.get("donchian_mid", 0)) for s, d in syms_data.items() if d.get("donchian_mid")]
            if _hb_total > 0:
                items.append(f" HOLD带  | SYS-030 Phase2 | 拦截中:{_hb_active}/{_hb_total}品种 "
                             f"| 中线追踪:{len(_hb_mids)}品种")

        # 8. Vision N字结构 (SYS-031)
        _np_file = "state/vision/pattern_latest.json"
        if os.path.exists(_np_file):
            with open(_np_file, 'r', encoding='utf-8') as f:
                _np_data = json.load(f)
            _up_n = sum(1 for s, d in _np_data.items() if isinstance(d, dict) and d.get("n_pattern") == "UP_N")
            _dn_n = sum(1 for s, d in _np_data.items() if isinstance(d, dict) and d.get("n_pattern") == "DOWN_N")
            if _up_n > 0 or _dn_n > 0:
                items.append(f" N字结构 | SYS-031 Phase1 | UP_N:{_up_n} DOWN_N:{_dn_n}")

        # 9. KEY-001 N字门控 (v3.660: 全品种ACTIVE, HOLD带暂停)
        _nsg_file = "state/n_structure_state.json"
        if os.path.exists(_nsg_file):
            with open(_nsg_file, 'r', encoding='utf-8') as f:
                _nsg = json.load(f)
            if _nsg:
                _state_counts = {}
                _active_count = sum(1 for _d in _nsg.values() if _d.get("mode") == "ACTIVE")
                _observe_count = sum(1 for _d in _nsg.values() if _d.get("mode") != "ACTIVE")
                for _s, _d in _nsg.items():
                    _st = _d.get("state", "?")
                    _state_counts[_st] = _state_counts.get(_st, 0) + 1
                _state_str = " ".join(f"{k}:{v}" for k, v in sorted(_state_counts.items()))
                _trail_on = sum(1 for _d in _nsg.values() if _d.get("trailing"))
                _buy_block = sum(1 for _d in _nsg.values() if _d.get("buy_block"))
                _sell_block = sum(1 for _d in _nsg.values() if _d.get("sell_block"))
                items.append(f" N字门控 | KEY-001 | {len(_nsg)}品种 (ACTIVE:{_active_count} OBS:{_observe_count}) [{_state_str}] 移动止损:{_trail_on} BUY拦截:{_buy_block} SELL拦截:{_sell_block}")
                # 逐品种状态 (仅非SIDE显示)
                for _s, _d in sorted(_nsg.items()):
                    if _d.get("state") != "SIDE":
                        _mode_tag = "拦截" if _d.get("mode") == "ACTIVE" else "观察"
                        _gate_tag = f"B:{'X' if _d.get('buy_block') else 'O'} S:{'X' if _d.get('sell_block') else 'O'}"
                        items.append(f"   {_s:8s} | {_d.get('state'):14s} | dir={_d.get('direction','?'):4s} "
                                     f"[{_mode_tag} {_gate_tag}] @{_d.get('ts','')}")

        # 10. KEY-002 品种自适应 (Phase 1 数据收集)
        _k2_file = "state/key002_adaptive.json"
        if os.path.exists(_k2_file):
            try:
                with open(_k2_file, 'r', encoding='utf-8') as f:
                    _k2 = json.load(f)
                if _k2:
                    _k2_syms = len(_k2)
                    _k2_total = sum(v.get("total_count", 0) for v in _k2.values())
                    _k2_diffs = sum(v.get("diff_count", 0) for v in _k2.values())
                    _k2_rate = f"{_k2_diffs/_k2_total:.0%}" if _k2_total > 0 else "N/A"
                    _k2_ready = sum(1 for _v in _k2.values() if (_v.get("param_plan") or {}).get("ready_for_rollout"))
                    items.append(f" Adaptive | KEY-002 Phase1 | {_k2_syms}品种 样本:{_k2_total} 差异:{_k2_diffs} ({_k2_rate}) 可接入:{_k2_ready}")
            except Exception:
                pass

        # 11. KEY-002 N字每日统计 (完美N/不完美N/SIDE × 扫描/交易)
        _nd_file = "state/n_daily_stats.json"
        if os.path.exists(_nd_file):
            try:
                with open(_nd_file, 'r', encoding='utf-8') as f:
                    _nd = json.load(f)
                _nd_date = _nd.get("date", "?")
                _nd_syms = _nd.get("symbols", {})
                if _nd_syms:
                    items.append(f" N字统计 | {_nd_date} | {len(_nd_syms)}品种")
                    items.append(f"   {'品种':8s} | {'完美N':>12s} | {'不完美N':>12s} | {'SIDE':>12s} | 交易占比")
                    for _sym in sorted(_nd_syms.keys()):
                        _sd = _nd_syms[_sym]
                        _pn = _sd.get("perfect_n", {})
                        _in = _sd.get("imperfect_n", {})
                        _si = _sd.get("side", {})
                        _total_trades = _pn.get("trades", 0) + _in.get("trades", 0) + _si.get("trades", 0)
                        _total_scans = _pn.get("scans", 0) + _in.get("scans", 0) + _si.get("scans", 0)
                        if _total_scans == 0:
                            continue
                        _pn_str = f"{_pn.get('scans',0)}s/{_pn.get('trades',0)}t"
                        _in_str = f"{_in.get('scans',0)}s/{_in.get('trades',0)}t"
                        _si_str = f"{_si.get('scans',0)}s/{_si.get('trades',0)}t"
                        _ratio = f"{_pn.get('trades',0)}/{_total_trades}" if _total_trades > 0 else "-"
                        items.append(f"   {_sym:8s} | {_pn_str:>12s} | {_in_str:>12s} | {_si_str:>12s} | 完美N:{_ratio}")
            except Exception:
                pass

        # 12. KEY-002 外挂智能进化建议
        _evo_file = "state/plugin_evolution.json"
        if os.path.exists(_evo_file):
            try:
                with open(_evo_file, 'r', encoding='utf-8') as f:
                    _evo = json.load(f)
                _evo_advice = _evo.get("advice", {})
                _evo_phase = "Phase2" if _evo.get("adjustments") else "Phase1"
                _evo_total = sum(len(v) for v in _evo_advice.values())
                _evo_tighten = sum(1 for s in _evo_advice.values() for r in s.values() if r.get("action") == "TIGHTEN")
                _evo_loosen = sum(1 for s in _evo_advice.values() for r in s.values() if r.get("action") == "LOOSEN")
                if _evo_total > 0:
                    items.append(f" Evolve  | KEY-002 {_evo_phase} | {_evo_total}建议 (收紧:{_evo_tighten} 放宽:{_evo_loosen}) 更新:{_evo.get('last_update','?')}")
                    for _evo_sym, _evo_plugins in sorted(_evo_advice.items()):
                        for _evo_p, _evo_r in _evo_plugins.items():
                            _act = _evo_r.get("action", "?")
                            _rsn = _evo_r.get("reason", "")[:50]
                            if _act in ("TIGHTEN", "LOOSEN"):
                                items.append(f"   {_evo_sym:8s} | {_evo_p:12s} | {_act:7s} | {_rsn}")
            except Exception:
                pass

        # 13. KEY-006 Vision Radar 影子观察
        _radar_log = Path("state/vision_radar_log.jsonl")
        if _radar_log.exists():
            try:
                _rlines = _radar_log.read_text(encoding="utf-8").strip().split("\n")
                _rlines = [l for l in _rlines if l.strip()]
                _r_total = len(_rlines)
                _r_buy = 0; _r_sell = 0; _r_conf = 0; _r_none = 0
                _r_last_sym = ""; _r_last_final = ""
                for _rl in _rlines[-100:]:  # only scan last 100
                    try:
                        _rr = json.loads(_rl)
                        _final = _rr.get("final", "")
                        _sig = _rr.get("radar", {}).get("signal", "NONE")
                        if _sig == "BUY": _r_buy += 1
                        elif _sig == "SELL": _r_sell += 1
                        else: _r_none += 1
                        if "CONFIRMED" in _final and "UNCONFIRMED" not in _final:
                            _r_conf += 1
                        _r_last_sym = _rr.get("symbol", "")
                        _r_last_final = _final
                    except Exception:
                        pass
                items.append(f" Radar   | KEY-006 | 记录:{_r_total} BUY:{_r_buy} SELL:{_r_sell} NONE:{_r_none} 确认:{_r_conf} | 最近:{_r_last_sym}→{_r_last_final}")
            except Exception:
                pass

        if items:
            print("\n" + "=" * total_width)
            print(f" 系统改善监控 (v3.660 Phase 1/2)")
            print("-" * total_width)
            for item in items:
                print(item)
            print("=" * total_width)
...
W_SYM, W_REG, W_DIR, W_3WAY, W_VOTE = 7, 6, 4, 9, 8  # v3.640: W_SYM 8→7 (BTCUSDC=7字符)
W_CON, W_P1, W_L2, W_FIN, W_POS = 12, 4, 4, 4, 4  # v3.640: W_POS 6→4 (5/5只需3字符)
...
def _get_vision_pattern_cached(symbol):
    ...
    short_map = {
        ...
        "ASC_TRIANGLE": "AT↑", "DESC_TRIANGLE": "DT↓",
        "WEDGE_RISING": "W↑S", "WEDGE_FALLING": "W↓B",
    }
...
def print_main_table(all_data):
    """v3.640: 面板 — 品种|当前|x4|规则%|V%|x4D%|x4C%|L1用|x4用|信号|L2|形态|最终|仓位 (14列, 压缩宽度)"""
    # 读取dual_track校准器数据
    dual_track = {}
    try:
        dual_track = load_json_file(HUMAN_DUAL_TRACK_FILE) or {}
    ...
    # 列宽 (v3.640: 压缩总宽度, 14列约106字符)
    WT = 4    # trend columns (UP/DOWN/SIDE)
    WA = 3    # accuracy columns (0-100)
    WU = 8    # L1用/x4用 (缠(48%) / V(72%))
    WS = 4    # 信号
    WL = 4    # L2 (compact)

    cols = [("品种", W_SYM), ("当前", WT), ("x4", WT),
            ("R%", WA), ("V%", WA), ("xD%", WA), ("xC%", WA),
            ("L1用", WU), ("x4用", WU), ("信号", WS), ("L2", WL), ("形态", W_PAT), ("最终", W_FIN), ("仓位", W_POS)]
    ...
    def _acc(val, is_best, width=WA):
        if val < 0:
            return pad_to_width("---", width)
        t = f"{val:.0f}"
        return (C_GB + pad_to_width(t, width) + C_0) if is_best else pad_to_width(t, width)
    ...
    for d in all_data:
        ...
        _best_cur = max(r_pct, v_pct)
        r_str = _acc(r_pct, r_pct == _best_cur and r_pct > 0)
        v_str = _acc(v_pct, v_pct == _best_cur and v_pct > 0)

        # v3.640: L1用 — 缠论 vs Vision 谁准用谁
        if v_pct > r_pct and v_pct > 0:
            l1_src = f"V({v_pct:.0f}%)"
            l1_color = C_M
        elif r_pct > 0:
            l1_src = f"缠({r_pct:.0f}%)"
        ...
        # v3.640: x4准确率 + x4用 — 道氏 vs 缠论 谁准用谁
        x4d_total = dt_stats.get("x4_dow_total", 0)
        x4c_total = dt_stats.get("x4_chan_total", 0)
        x4d_pct = dt_stats.get("x4_dow_correct", 0) / x4d_total * 100 if x4d_total > 0 else -1
        x4c_pct = dt_stats.get("x4_chan_correct", 0) / x4c_total * 100 if x4c_total > 0 else -1
        _best_x4 = max(x4d_pct, x4c_pct)
        x4d_str = _acc(x4d_pct, x4d_pct == _best_x4 and x4d_pct > 0)
        x4c_str = _acc(x4c_pct, x4c_pct == _best_x4 and x4c_pct > 0)
        if x4c_pct > x4d_pct and x4c_pct > 0:
            x4_src = f"C({x4c_pct:.0f}%)"
            x4_color = C_C
        elif x4d_pct > 0:
            x4_src = f"D({x4d_pct:.0f}%)"
            x4_color = C_0
        else:
            x4_src = "D"
            x4_color = C_0
        ...
        row = ("| " + pad_to_width(symbol[:W_SYM], W_SYM)
               ...
               + " | " + x4d_str
               + " | " + x4c_str
               + " | " + _c(l1_src, WU, l1_color)
               + " | " + _c(x4_src, WU, x4_color)
               + " | " + sig_str
               + " | " + l2_str
               + " | " + fmt_pattern(d)
               + " | " + fmt_fin(d)
    ...
    print("-" * total_w)
    print(f"  R%=缠论  V%=Vision  xD%=x4道氏  xC%=x4缠论  L1用/x4用=谁准用谁  {C_GB}绿{C_0}=更高者")
...
def print_plugin_profit_report():
    ...
    for sym, entries in open_entries.items():
        if not entries:
            continue
        # 获取当前价格
        sym_state = main_state.get(sym, {})
        cur_price = sym_state.get("_last_final_decision", {}).get("current_price", 0)

        asset = entries[0].get("asset_type", "crypto")
        parts_list = crypto_open if asset == "crypto" else stock_open
        for e in entries:
            p = e.get("plugin", "?")
            buy_price = e.get("price", 0)
            ...
            if cur_price > 0 and buy_price > 0:
                ...
                has_unrealized = True
                if unrealized >= 0:
                    pnl_tag = f"{C_G}+${unrealized:,.2f}{C_0}"
                else:
                    pnl_tag = f"{C_R}-${abs(unrealized):,.2f}{C_0}"
                parts_list.append(f"{p}({sym}) {pnl_tag}")
            else:
                parts_list.append(f"{p}({sym})x1")
    ...
    if has_unrealized:
        if total_unrealized >= 0:
            total_str = f"{C_GB}+${total_unrealized:,.2f}{C_0}"
        else:
            total_str = f"{C_RB}-${abs(total_unrealized):,.2f}{C_0}"
        print(f" 浮动盈亏: {total_str}")
    print("=" * total_width)
...
_IMP_STATUS_ICONS = {
    "FOUND": "?", "ANALYZED": "#", "IN_PROGRESS": ">",
    "TESTING": "T", "VERIFIED": "V", "CLOSED": "X", "DEFERRED": "D",
}
_IMP_STATUS_ORDER = ["FOUND", "ANALYZED", "IN_PROGRESS", "TESTING", "VERIFIED", "CLOSED"]
_IMP_PRIORITY_ORDER = ["P0", "P1", "P2", "P3"]


def _imp_checklist_bar(cl):
    """4格进度条: analyzed->coded->tested->verified"""
    ...
    return f"{bar} {labels[-1] if labels else 'found'}"


def _imp_truncate(text, max_dw):
    """按显示宽度截断, 超长加.."""
    if display_width(text) <= max_dw:
        return text
    t = ""
    for c in text:
        if display_width(t + c) > max_dw - 2:
            break
        t += c
    return t + ".."


def _imp_layer_bar(items, layer, bar_len=15):
    """生成单层进度条: [########.......] 50% (5/10)"""
    li = [i for i in items if i.get("layer") == layer]
    total = len(li)
    closed = len([i for i in li if i.get("status") == "CLOSED"])
    pct = (closed / total * 100) if total > 0 else 0
    filled = int(pct / 100 * bar_len)
    bar = "#" * filled + "." * (bar_len - filled)
    active = total - closed
    return f"[{bar}] {pct:4.0f}% ({closed}/{total}done, {active}active)"


def print_improvement_tracker():
    """v3.650: IMPROVEMENT TRACKER — 三层改善追踪面板"""
    data = load_json_file(IMPROVEMENTS_FILE)
    if not data:
        return
    ...
    W_id = 9        # "TOOL-010 "
    W_p = 2         # "P0"
    W_title = 42    # 中文标题+Phase
    W_status = 13   # "> IN_PROGRESS"
    W_prog = 15     # "#### verified"

    # 计算总宽: "| col | col | col | col | col |"
    total_width = 2 + W_id + 3 + W_p + 3 + W_title + 3 + W_status + 3 + W_prog + 2
    sep = "-" * total_width

    print("\n" + "=" * total_width)
    print(" IMPROVEMENT TRACKER (v3.650)")
    print(sep)

    # --- Top: per-layer progress bars ---
    for layer in ["SYSTEM", "TOOL", "AUDIT", "RESEARCH"]:
        li = [i for i in items if i.get("layer") == layer]
        active_cnt = len([i for i in li if i["status"] != "CLOSED"])
        print(f"  {pad_to_width(layer, 8)}({active_cnt:>2})  {_imp_layer_bar(items, layer)}")
    print(sep)

    # --- Main table: active items sorted by priority ---
    active = [i for i in items if i["status"] != "CLOSED"]
    pri_ord = {p: idx for idx, p in enumerate(_IMP_PRIORITY_ORDER)}
    sta_ord = {s: idx for idx, s in enumerate(_IMP_STATUS_ORDER)}
    active.sort(key=lambda x: (pri_ord.get(x["priority"], 9), sta_ord.get(x["status"], 9)))

    # Header — 全部用 pad_to_width 保证对齐
    hdr = "| " + pad_to_width("ID", W_id) + " | " + pad_to_width("P", W_p) + " | "
    hdr += pad_to_width("标题", W_title) + " | "
    hdr += pad_to_width("状态", W_status) + " | "
    hdr += pad_to_width("进度", W_prog) + " |"
    print(hdr)
    print(sep)

    for item in active:
        pcol = C_R if item["priority"] == "P0" else (C_Y if item["priority"] == "P1" else C_W)
        scol = ""
        if item["status"] in ("VERIFIED",):
            scol = C_G
        elif item["status"] == "IN_PROGRESS":
            scol = C_C
        elif item["status"] in ("FOUND", "ANALYZED"):
            scol = C_Y

        icon = _IMP_STATUS_ICONS.get(item["status"], "?")

        # Title: phase用显示宽度截断再拼接, 最后整体截断
        title = item["title"]
        if item.get("phase"):
            phase_short = _imp_truncate(item["phase"], 14)
            title += " [" + phase_short + "]"
        title = _imp_truncate(title, W_title)

        status_text = icon + " " + item["status"]
        bar = _imp_checklist_bar(item.get("checklist", {}))

        # 每列都用 pad_to_width, ANSI颜色包在外面
        row = "| " + pad_to_width(item["id"], W_id) + " | "
        row += pcol + pad_to_width(item["priority"], W_p) + C_0 + " | "
        row += pad_to_width(title, W_title) + " | "
        row += scol + pad_to_width(status_text, W_status) + C_0 + " | "
        row += pad_to_width(bar, W_prog) + " |"
        print(row)
    ...
    # --- Bottom stats ---
    pri_parts = []
    for p in _IMP_PRIORITY_ORDER:
        pi = [i for i in active if i["priority"] == p]
        if pi:
            statuses = {}
            for i in pi:
                statuses[i["status"]] = statuses.get(i["status"], 0) + 1
            detail = " ".join(f"{_IMP_STATUS_ICONS[s]}{v}" for s, v in statuses.items())
            pcol = C_R if p == "P0" else (C_Y if p == "P1" else C_W)
            pri_parts.append(f"{pcol}{p}{C_0}:{len(pi)}({detail})")
    print(" " + "  ".join(pri_parts))
    ...
    try:
        ...
        for i in active:
            try:
                ud = dt.strptime(i.get("updated_date", ""), "%Y-%m-%d")
                if (now - ud).days > 7:
                    stale.append(i["id"])
            except (ValueError, TypeError):
                pass
        stale_str = f"  {C_R}阻塞: {','.join(stale)}{C_0}" if stale else ""
        print(f" 本周关闭: {len(week_closed)}  活跃: {len(active)}/{len(items)}{stale_str}")
    except Exception:
        print(f" 活跃: {len(active)}/{len(items)}")

    print(sep)


def print_dualpath_status():
    """GCC-0146: DualPath性能监控面板"""
    try:
        baseline_file = os.path.join(os.path.dirname(__file__), "state", "dualpath_baseline.json")
        ...
        print("\n" + "=" * 120)
        print(f" DualPath 性能监控 (GCC-0146) | {len(rounds)} rounds | updated: {updated}")
        print("-" * 120)

        # 关键指标
        scan_total = summary.get("scan_once_total", {})
        bv = summary.get("brooks_vision", {})

        if scan_total:
            print(f"  scan_once_total: avg={scan_total.get('avg_ms',0):.0f}ms "
                  f"p50={scan_total.get('p50_ms',0):.0f}ms "
                  f"p95={scan_total.get('p95_ms',0):.0f}ms "
                  f"max={scan_total.get('max_ms',0):.0f}ms "
                  f"(n={scan_total.get('count',0)})")
        if bv:
            print(f"  brooks_vision:   avg={bv.get('avg_ms',0):.0f}ms "
                  f"p50={bv.get('p50_ms',0):.0f}ms "
                  f"p95={bv.get('p95_ms',0):.0f}ms")

        # 品种级别: 找出最慢的3个plugins和price
        plugin_items = [(k, v) for k, v in summary.items() if k.startswith("plugins_")]
        price_items = [(k, v) for k, v in summary.items() if k.startswith("price_")]
        knn_items = [(k, v) for k, v in summary.items() if k.startswith("knn_")]

        if plugin_items:
            top3 = sorted(plugin_items, key=lambda x: x[1].get("avg_ms", 0), reverse=True)[:3]
            parts = [f"{k.replace('plugins_','')}: {v.get('avg_ms',0):.0f}ms" for k, v in top3]
            print(f"  plugins TOP3:    {' | '.join(parts)}")

        if price_items:
            top3 = sorted(price_items, key=lambda x: x[1].get("avg_ms", 0), reverse=True)[:3]
            parts = [f"{k.replace('price_','')}: {v.get('avg_ms',0):.0f}ms" for k, v in top3]
            print(f"  price TOP3:      {' | '.join(parts)}")

        if knn_items:
            top3 = sorted(knn_items, key=lambda x: x[1].get("avg_ms", 0), reverse=True)[:3]
            parts = [f"{k.replace('knn_','')}: {v.get('avg_ms',0):.1f}ms" for k, v in top3]
            print(f"  knn TOP3:        {' | '.join(parts)}")

        # Traffic stats
        traffic_sig = summary.get("traffic_signal_count", {})
        traffic_blk = summary.get("traffic_relay_blocked", {})
        if traffic_sig:
            print(f"  traffic:         signals={traffic_sig.get('avg_ms',0):.0f} "
                  f"relay_blocked={traffic_blk.get('avg_ms',0):.0f}")

        # Prefetch stats
        prefetch_items = [(k, v) for k, v in summary.items() if k.startswith("prefetch_")]
        if prefetch_items:
            total_prefetch = sum(v.get("count", 0) for _, v in prefetch_items)
            avg_prefetch = sum(v.get("avg_ms", 0) for _, v in prefetch_items) / max(len(prefetch_items), 1)
            print(f"  prefetch:        {total_prefetch} total, avg={avg_prefetch:.0f}ms/item")
...
def print_testing_verification():
    """v3.654: 全量改善运行状态面板 — TESTING验证+IN_PROGRESS观察"""
    import re as _re

    imp_data = load_json_file(IMPROVEMENTS_FILE)
    if not imp_data:
        return
    items = imp_data.get("items", [])
    active_items = [i for i in items if i.get("status") in ("TESTING", "IN_PROGRESS")]
    if not active_items:
        return

    # --- 收集实时数据 ---
    live_data = _collect_improvement_live_data()

    total_width = 92
    sep = "-" * total_width

    print("\n" + "=" * total_width)
    print(" IMPROVEMENT LIVE STATUS (v3.654)")
    print(sep)

    # 分组: TESTING先, IN_PROGRESS后
    testing = [i for i in active_items if i["status"] == "TESTING"]
    in_progress = [i for i in active_items if i["status"] == "IN_PROGRESS"]
    ...
    hdr = "| " + pad_to_width("ID", W_id) + " | "
    hdr += pad_to_width("标题", W_title) + " | "
    hdr += pad_to_width("阶段", W_phase) + " | "
    hdr += pad_to_width("状态", W_verdict) + " | "
    hdr += pad_to_width("实时数据", W_detail) + " |"
    print(hdr)
    print(sep)

    # --- TESTING 组 ---
    if testing:
        print(f" {C_Y}[ TESTING — 待日志验证 ]{C_0}")
        for item in testing:
            _print_live_row(item, live_data, W_id, W_title, W_phase, W_verdict, W_detail)

    # --- IN_PROGRESS 组 ---
    if in_progress:
        if testing:
            print(sep)
        print(f" {C_C}[ IN_PROGRESS — Phase 1 观察 ]{C_0}")
        pri_ord = {"P0": 0, "P1": 1, "P2": 2, "P3": 3}
        in_progress.sort(key=lambda x: pri_ord.get(x.get("priority", "P3"), 9))
    ...
    # --- 汇总 ---
    t_cnt = len(testing)
    p_cnt = len(in_progress)
    active_v = sum(1 for i in active_items if live_data.get(i["id"], {}).get("verdict") == "ACTIVE")
    deployed = sum(1 for i in active_items if live_data.get(i["id"], {}).get("verdict") == "DEPLOYED")
    pending = sum(1 for i in active_items if live_data.get(i["id"], {}).get("verdict") == "PENDING")
    nodata = sum(1 for i in active_items if live_data.get(i["id"], {}).get("verdict") == "NO_DATA")
    print(f" TESTING:{t_cnt} IN_PROGRESS:{p_cnt}  |  "
          f"{C_G}ACTIVE:{active_v}{C_0}  {C_C}DEPLOYED:{deployed}{C_0}  {C_Y}PENDING:{pending}{C_0}  {C_R}NO_DATA:{nodata}{C_0}")
    print(sep)


def _collect_improvement_live_data():
    """收集各改善项的实时运行数据"""
    data = {}

    # --- 读取各state文件 ---
    orderflow = load_json_file("state/orderflow_state.json")
    trade_freq = load_json_file("state/trade_frequency.json")
    strategy_m = load_json_file("state/strategy_metrics.json")
    human_dt = load_json_file("state/human_dual_track.json")
    stock_sel = load_json_file("state/stock_selection.json")
    corr_state = load_json_file("state/correlation_state.json")
    dtw_state = load_json_file("state/dtw_patterns.json")
    breaker_st = load_json_file("state/portfolio_breaker.json")

    # --- 读server.log最后500行搜关键词 ---
    log_counts = _scan_log_for_improvements()

    # === TESTING items (RES-001~006) — coded=true, 等待日志验证 ===
    # RES-001: 节奏阈值品种分化
    cnt = log_counts.get("rhythm_crypto", 0)
    data["RES-001"] = {"verdict": "ACTIVE" if cnt > 0 else "DEPLOYED",
                       "detail": f"加密节奏过滤:{cnt}次" if cnt > 0 else "已部署,等待日志同步"}

    # RES-002: Vision过度自信降权
    cnt = log_counts.get("vision_overconf", 0)
    data["RES-002"] = {"verdict": "ACTIVE" if cnt > 0 else "DEPLOYED",
                       "detail": f"降权触发:{cnt}次" if cnt > 0 else "已部署,等待Vision触发"}

    # RES-003: 校准器晚期融合诊断
    has_fusion = False
    if human_dt:
        for sym_data in human_dt.values():
            if isinstance(sym_data, dict) and sym_data.get("late_fusion_score") is not None:
                has_fusion = True
                break
    data["RES-003"] = {"verdict": "ACTIVE" if has_fusion else "DEPLOYED",
                       "detail": "融合数据可用" if has_fusion else "已部署,等待校准器运行"}

    # RES-004: 动态τ过滤
    cnt = log_counts.get("tau_filter", 0)
    data["RES-004"] = {"verdict": "ACTIVE" if cnt > 0 else "DEPLOYED",
                       "detail": f"τ过滤:{cnt}次" if cnt > 0 else "已部署,等待校准器样本"}

    # RES-005: 视角分歧度
    cnt = log_counts.get("divergence", 0)
    data["RES-005"] = {"verdict": "ACTIVE" if cnt > 0 else "DEPLOYED",
                       "detail": f"分歧度触发:{cnt}次" if cnt > 0 else "已部署,等待分歧度数据"}

    # RES-006: EMA窗口品种分化
    cnt = log_counts.get("ema_diff", 0)
    data["RES-006"] = {"verdict": "ACTIVE" if cnt > 0 else "DEPLOYED",
                       "detail": f"EMA分化:{cnt}次" if cnt > 0 else "已部署,等待EMA5触发"}

    # === IN_PROGRESS items ===
    # SYS-006: Module B 交易频率控制
    if trade_freq:
        used = trade_freq.get("global_used", 0)
        mode = trade_freq.get("mode", "?")
        data["SYS-006"] = {"verdict": "ACTIVE", "detail": f"{mode} {used}笔"}
    else:
        data["SYS-006"] = {"verdict": "NO_DATA", "detail": "无state文件"}

    # SYS-007: Module C 反思迭代
    if stock_sel:
        grades = stock_sel.get("grades") or stock_sel.get("scores") or {}
        if isinstance(grades, dict) and grades:
            n_grades = len(grades)
            data["SYS-007"] = {"verdict": "ACTIVE", "detail": f"已评{n_grades}品种"}
        else:
            data["SYS-007"] = {"verdict": "PENDING", "detail": "等待收盘审计输出(grades/scores)"}
    else:
        data["SYS-007"] = {"verdict": "PENDING", "detail": "等待stock_selection状态文件"}

    # SYS-011: 跨周期共识度
    cnt = log_counts.get("consensus", 0)
    data["SYS-011"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"共识评分:{cnt}次" if cnt > 0 else "等待扫描引擎"}

    # KEY-001: N字结构门控 (v3.660: 全品种ACTIVE, HOLD带暂停)
    nsg = load_json_file("state/n_structure_state.json")
    if nsg and len(nsg) > 0:
        _states = {}
        _active = sum(1 for _d in nsg.values() if _d.get("mode") == "ACTIVE")
        for _d in nsg.values():
            _st = _d.get("state", "?")
            _states[_st] = _states.get(_st, 0) + 1
        _st_str = " ".join(f"{k}:{v}" for k, v in _states.items())
        _ngba_cnt = log_counts.get("n_gate_block_active", 0)
        _vn_cnt = log_counts.get("vision_n_conflict", 0)
        _buy_block = sum(1 for _d in nsg.values() if _d.get("buy_block"))
        _sell_block = sum(1 for _d in nsg.values() if _d.get("sell_block"))
        _block_tag = f" 今日拦截:{_ngba_cnt}" if _ngba_cnt > 0 else ""
        _vn_tag = f" Vision冲突:{_vn_cnt}" if _vn_cnt > 0 else ""
        data["KEY-001"] = {"verdict": "ACTIVE", "detail": f"{len(nsg)}品种(ACTIVE:{_active}) {_st_str} BUY拦截:{_buy_block} SELL拦截:{_sell_block}{_block_tag}{_vn_tag}"}
    else:
        data["KEY-001"] = {"verdict": "DEPLOYED", "detail": "已部署,等待首次运行"}

    # KEY-002: 品种自适应 (Phase 1 数据收集)
    k2_data = load_json_file("state/key002_adaptive.json")
    if k2_data and len(k2_data) > 0:
        _k2_total = sum(v.get("total_count", 0) for v in k2_data.values())
        _k2_diff = sum(v.get("diff_count", 0) for v in k2_data.values())
        _k2_rate = f"{_k2_diff/_k2_total:.0%}" if _k2_total > 0 else "N/A"
        _k2_ready = sum(1 for _v in k2_data.values() if (_v.get("param_plan") or {}).get("ready_for_rollout"))
        data["KEY-002"] = {"verdict": "COLLECTING", "detail": f"{len(k2_data)}品种 样本:{_k2_total} 差异:{_k2_diff}({_k2_rate}) 可接入:{_k2_ready}"}
    else:
        data["KEY-002"] = {"verdict": "DEPLOYED", "detail": "已部署,等待首次运行"}

    # KEY-003: 价值分析模块 + 后5名限买 (v3.663)
    k3_state = load_json_file("state/value_analysis_validation.json")
    k3_latest = load_json_file("state/value_analysis_latest.json")
    k3_module_cnt = log_counts.get("key003_validate", 0)
    k3_fail_cnt = log_counts.get("key003_fail", 0)
    k3_vg_block = log_counts.get("key003_value_guard_block", 0)
    # 从latest提取后5名
    k3_bottom5 = []
    if k3_latest and isinstance(k3_latest.get("results"), list):
        k3_valid = [r for r in k3_latest["results"] if "composite_score" in r and not r["ticker"].endswith("USDC")]
        k3_valid.sort(key=lambda x: x["composite_score"])
        k3_bottom5 = [f"{r['ticker']}({r['composite_score']:.1f})" for r in k3_valid[:5]]
    if k3_state:
        k3_passed = int(k3_state.get("passed_modules", 0))
        k3_total = int(k3_state.get("total_modules", 0))
        k3_overall = str(k3_state.get("overall", "UNKNOWN"))
        k3_last = str(k3_state.get("last_run", ""))
        k3_detail = f"验证:{k3_passed}/{k3_total} "
        if k3_bottom5:
            k3_detail += f"限买后5: {','.join(k3_bottom5)} "
        if k3_vg_block > 0:
            k3_detail += f"BUY拦截:{k3_vg_block}次"
        data["KEY-003"] = {
            "verdict": "ACTIVE" if k3_total > 0 and k3_passed == k3_total else "PENDING",
            "detail": k3_detail,
        }
    else:
        k3_detail = ""
        ...
        if k3_vg_block > 0:
            k3_detail += f"BUY拦截:{k3_vg_block}次"
        data["KEY-003"] = {
            "verdict": "DEPLOYED" if k3_latest else "PENDING",
            "detail": k3_detail or "等待首次运行",
        }

    # KEY-004: 外挂品质治理与评估体系
    k4_state = load_json_file("state/plugin_profit_state.json")
    if not k4_state:
        k4_state = load_json_file("plugin_profit_state.json")
    if k4_state and isinstance(k4_state, dict):
        k4_quality = k4_state.get("quality_snapshot", {}) if isinstance(k4_state.get("quality_snapshot", {}), dict) else {}
        k4_crypto = k4_quality.get("crypto", []) if isinstance(k4_quality, dict) else []
        k4_stock = k4_quality.get("stock", []) if isinstance(k4_quality, dict) else []
        k4_all = []
        if isinstance(k4_crypto, list):
            k4_all.extend(k4_crypto)
        if isinstance(k4_stock, list):
            k4_all.extend(k4_stock)
        k4_all = [r for r in k4_all if isinstance(r, dict) and isinstance(r.get("quality_score"), (int, float))]

        if k4_all:
            k4_all.sort(key=lambda x: float(x.get("quality_score", 0.0)))
            _k4_min = k4_all[0]
            _k4_avg = sum(float(r.get("quality_score", 0.0)) for r in k4_all) / len(k4_all)
            _k4_evt = log_counts.get("key004_plugin_event", 0)
            # T06: 治理+N字转换
            _k4_gov = log_counts.get("key004_governance", 0)
            _k4_nt = log_counts.get("key004_n_transition", 0)
            _k4_ne = log_counts.get("key004_n_eval", 0)
            # 治理动作分布
            k4_gov_data = load_json_file("state/plugin_governance_actions.json")
            _k4_dis = 0
            _k4_obs = 0
            if k4_gov_data and isinstance(k4_gov_data, dict):
                for _at in ("crypto", "stock"):
                    for _r in k4_gov_data.get("by_asset", {}).get(_at, []):
                        if isinstance(_r, dict):
                            _d = _r.get("decision", "")
                            if _d == "DISABLE_CANDIDATE":
                                _k4_dis += 1
                            elif _d == "OBSERVE_OR_DOWNWEIGHT":
                                _k4_obs += 1
            # N字转换可靠性
            k4_nt_data = load_json_file("state/n_transition_events.json")
            _k4_nt_today = 0
            _k4_rel_str = ""
            if k4_nt_data and isinstance(k4_nt_data, dict):
                _k4_nt_today = len(k4_nt_data.get("events", []))
                _rel = k4_nt_data.get("reliability", {})
                if _rel:
                    _total_c = sum(v.get("correct", 0) for sym_r in _rel.values() for v in sym_r.values())
                    _total_t = sum(v.get("total", 0) for sym_r in _rel.values() for v in sym_r.values())
                    if _total_t > 0:
                        _k4_rel_str = f" 可靠性:{_total_c}/{_total_t}({_total_c/_total_t*100:.0f}%)"
            _extra = f" 治理:{_k4_gov}(DIS:{_k4_dis}/OBS:{_k4_obs}) N转换:{_k4_nt_today}{_k4_rel_str}"
            data["KEY-004"] = {
                "verdict": "ACTIVE",
                "detail": f"{len(k4_all)}外挂 平均分:{_k4_avg:.1f} 最低:{_k4_min.get('plugin','?')}({_k4_min.get('quality_score',0):.1f}) 事件:{_k4_evt}{_extra}",
            }
        else:
            data["KEY-004"] = {"verdict": "DEPLOYED", "detail": "已部署评分模型,等待交易样本"}
    else:
        data["KEY-004"] = {"verdict": "PENDING", "detail": "等待plugin_profit_state产出"}

    # KEY-005: 行为金融增强层 (CSI/DQS + A/B归因)
    k5_runtime = load_json_file("state/behavior_finance_runtime.json")
    k5_ab = load_json_file("state/behavior_ab_metrics.json")
    k5_diag = load_json_file("state/behavior_diagnostics.json")
    if k5_runtime and isinstance(k5_runtime.get("symbols"), dict) and k5_runtime["symbols"]:
        _k5_symbols = k5_runtime.get("symbols", {})
        _k5_dqs_vals = []
        _k5_anchor_strength = []
        _k5_states = {}
        _k5_hii_states = {}
        _k5_risk_modes = {}
        for _sym, _payload in _k5_symbols.items():
            _dqs = _payload.get("dqs")
            if isinstance(_dqs, (int, float)):
                _k5_dqs_vals.append(float(_dqs))
            _st = _payload.get("csi_state", "NEUTRAL")
            _k5_states[_st] = _k5_states.get(_st, 0) + 1
            _hii_st = _payload.get("hii_state", "BALANCED")
            _k5_hii_states[_hii_st] = _k5_hii_states.get(_hii_st, 0) + 1
            _nearest = _payload.get("anchor_map", {}).get("nearest", {}) if isinstance(_payload, dict) else {}
            _stg = _nearest.get("strength") if isinstance(_nearest, dict) else None
            if isinstance(_stg, (int, float)):
                _k5_anchor_strength.append(float(_stg))
            _risk_mode = _payload.get("signal_modulation", {}).get("risk_mode", "NORMAL") if isinstance(_payload, dict) else "NORMAL"
            _k5_risk_modes[_risk_mode] = _k5_risk_modes.get(_risk_mode, 0) + 1

        _k5_avg_dqs = (sum(_k5_dqs_vals) / len(_k5_dqs_vals)) if _k5_dqs_vals else 0.0
        _k5_avg_anchor = (sum(_k5_anchor_strength) / len(_k5_anchor_strength)) if _k5_anchor_strength else 0.0
        _k5_state_str = " ".join(f"{k}:{v}" for k, v in sorted(_k5_states.items()))
        _k5_hii_state_str = " ".join(f"{k}:{v}" for k, v in sorted(_k5_hii_states.items()))
        _k5_risk_str = " ".join(f"{k}:{v}" for k, v in sorted(_k5_risk_modes.items()))

        _k5_treat = 0
        _k5_block = 0
        _k5_ac_fail = 0
        if k5_ab and isinstance(k5_ab.get("global"), dict):
            _k5_treat = int(k5_ab["global"].get("treatment_decisions", 0))
            _k5_block = int(k5_ab["global"].get("blocked_new_entry", 0))
        if k5_diag and isinstance(k5_diag.get("symbols"), dict):
            for _diag in k5_diag.get("symbols", {}).values():
                _stats = _diag.get("stats", {}) if isinstance(_diag, dict) else {}
                _k5_ac_fail += int(_stats.get("anti_cheat_fail", 0))

        data["KEY-005"] = {
            "verdict": "ACTIVE",
            "detail": f"{len(_k5_symbols)}品种 CSI[{_k5_state_str}] HII[{_k5_hii_state_str}] Risk[{_k5_risk_str}] DQS均值:{_k5_avg_dqs:.1f} Anchor均强:{_k5_avg_anchor:.1f} A/B干预:{_k5_treat} 禁开仓:{_k5_block} AC失败:{_k5_ac_fail}",
        }
    else:
        data["KEY-005"] = {
            "verdict": "DEPLOYED",
            "detail": "已部署,等待行为金融首轮运行",
        }

    # KEY-006: Vision Radar 影子观察系统
    _radar_log = Path("state/vision_radar_log.jsonl")
    if _radar_log.exists():
        try:
            _radar_lines = _radar_log.read_text(encoding="utf-8").strip().split("\n")
            _radar_lines = [l for l in _radar_lines if l.strip()]
            _r_total = len(_radar_lines)
            _r_signals = 0
            _r_confirmed = 0
            _r_unconfirmed = 0
            _r_none = 0
            for _rl in _radar_lines:
                try:
                    _rr = json.loads(_rl)
                    _final = _rr.get("final", "")
                    if "CONFIRMED" in _final and "UNCONFIRMED" not in _final:
                        _r_confirmed += 1
                        _r_signals += 1
                    elif "UNCONFIRMED" in _final:
                        _r_unconfirmed += 1
                        _r_signals += 1
                    elif "PARTIAL" in _final or "VOL_REJECT" in _final or "MASTER_REJECT" in _final:
                        _r_signals += 1
                    elif "NO_SIGNAL" in _final:
                        _r_none += 1
                except Exception:
                    pass
            data["KEY-006"] = {
                "verdict": "ACTIVE",
                "detail": f"记录:{_r_total} 信号:{_r_signals}(确认:{_r_confirmed} 未确认:{_r_unconfirmed}) 无信号:{_r_none}",
            }
        except Exception:
            data["KEY-006"] = {"verdict": "ACTIVE", "detail": "JSONL读取异常"}
    else:
        data["KEY-006"] = {"verdict": "DEPLOYED", "detail": "等待首轮Radar扫描"}

    # KEY-007: Plugin KNN 外挂因子化进化
    _knn_hist = Path("state/plugin_knn_history.npz")
    if _knn_hist.exists():
        try:
            import numpy as np
            _knn_data = np.load(str(_knn_hist), allow_pickle=True)
            _knn_keys = [k for k in _knn_data.files if k.endswith("_features")]
            _knn_total_samples = sum(len(_knn_data[k]) for k in _knn_keys) if _knn_keys else 0
            data["KEY-007"] = {"verdict": "ACTIVE", "detail": f"{len(_knn_keys)}库 样本:{_knn_total_samples}"}
        except Exception:
            data["KEY-007"] = {"verdict": "DEPLOYED", "detail": "npz读取异常"}
    else:
        data["KEY-007"] = {"verdict": "DEPLOYED", "detail": "等待KNN首轮记录"}

    # SYS-015: SQS量价门槛
    if orderflow:
        n_sym = len(orderflow)
        blocked = sum(v.get("blocked_count", 0) for v in orderflow.values() if isinstance(v, dict))
        data["SYS-015"] = {"verdict": "ACTIVE", "detail": f"{n_sym}品种 拦截:{blocked}"}
    else:
        data["SYS-015"] = {"verdict": "NO_DATA", "detail": "无OrderFlow数据"}

    # SYS-016: 外挂VF统一过滤
    if orderflow:
        vf_active = sum(1 for v in orderflow.values()
                        if isinstance(v, dict) and v.get("vf") not in (None, "N/A"))
        data["SYS-016"] = {"verdict": "ACTIVE" if vf_active > 0 else "PENDING",
                           "detail": f"VF有效:{vf_active}品种" if vf_active > 0 else "VF全N/A"}
    else:
        data["SYS-016"] = {"verdict": "NO_DATA", "detail": "无OrderFlow数据"}

    # SYS-017: 连续亏损熔断
    if breaker_st:
        state = breaker_st.get("state", "NORMAL")
        data["SYS-017"] = {"verdict": "ACTIVE", "detail": f"状态:{state}"}
    else:
        cnt_b = log_counts.get("breaker", 0)
        data["SYS-017"] = {"verdict": "ACTIVE" if cnt_b > 0 else "PENDING",
                           "detail": f"熔断记录:{cnt_b}" if cnt_b > 0 else "等待熔断日志或portfolio_breaker状态"}

    # SYS-018: StrategyEvaluator
    if strategy_m and strategy_m.get("metrics"):
        wr = strategy_m["metrics"].get("win_rate", 0)
        pf = strategy_m["metrics"].get("profit_factor", 0)
        data["SYS-018"] = {"verdict": "ACTIVE", "detail": f"WR:{wr:.1%} PF:{pf:.2f}"}
    else:
        data["SYS-018"] = {"verdict": "NO_DATA", "detail": "无strategy_metrics"}

    # SYS-019: 组合相关性风控
    if corr_state:
        data["SYS-019"] = {"verdict": "ACTIVE", "detail": "相关性矩阵可用"}
    else:
        data["SYS-019"] = {"verdict": "NO_DATA", "detail": "无correlation数据"}

    # SYS-020: 共识权重动态再平衡
    cnt = log_counts.get("regime_weight", 0)
    data["SYS-020"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"Regime权重:{cnt}次" if cnt > 0 else "等待Regime数据"}

    # SYS-022: 置信度门控 (DEFERRED但保留检测)
    cnt = log_counts.get("confidence_gate", 0)
    data["SYS-022"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"门控触发:{cnt}次" if cnt > 0 else "等待置信度门控日志(confidence_gate)"}

    # RES-007: 趋势阶段风险预算+市场制度 (v21.12 FINSABER增强)
    cnt = log_counts.get("trend_phase", 0)
    _rhythm_cnt = log_counts.get("phase_rhythm", 0) if "phase_rhythm" in log_counts else 0
    data["RES-007"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"阶段:{cnt}次 阈值调整:{_rhythm_cnt}次" if cnt > 0 else "等待趋势阶段日志(trend_phase)"}

    # RES-008: 持仓时间衰减
    cnt = log_counts.get("time_decay", 0)
    data["RES-008"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"衰减观察:{cnt}次" if cnt > 0 else "等待时间衰减日志(time_decay/持仓>6h)"}

    # SYS-025: 趋势健康度
    cnt = log_counts.get("trend_health", 0)
    data["SYS-025"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"健康度记录:{cnt}次" if cnt > 0 else "等待趋势健康度日志(trend_health)"}

    # SYS-026: Donchian量价增强
    cnt = log_counts.get("donchian_vol", 0)
    data["SYS-026"] = {"verdict": "ACTIVE" if cnt > 0 else "PENDING",
                       "detail": f"量价记录:{cnt}次" if cnt > 0 else "等待Donchian量价日志(donchian_vol)"}

    # SYS-031: Vision N字结构
    np_file = "state/vision/pattern_latest.json"
    np_data = load_json_file(np_file)
    if np_data:
        _up = sum(1 for d in np_data.values() if isinstance(d, dict) and d.get("n_pattern") == "UP_N")
        _dn = sum(1 for d in np_data.values() if isinstance(d, dict) and d.get("n_pattern") == "DOWN_N")
        data["SYS-031"] = {"verdict": "ACTIVE" if (_up + _dn) > 0 else "DEPLOYED",
                           "detail": f"UP_N:{_up} DOWN_N:{_dn}" if (_up + _dn) > 0 else "已部署,等待Vision"}
    else:
        data["SYS-031"] = {"verdict": "DEPLOYED", "detail": "已部署,等待Vision触发"}

    # AUD-049/050: 翻转问题 — 依赖SYS-006/017效果
    data["AUD-049"] = {"verdict": "DEPLOYED", "detail": "依赖SYS-006/017+KEY-001"}
    data["AUD-050"] = {"verdict": "DEPLOYED", "detail": "依赖SYS-006/017+KEY-001"}

    return data


def _scan_log_for_improvements():
    """扫描server.log最后2000行, 统计各改善项关键词出现次数"""
    import re as _re
    counts = {
        "rhythm_crypto": 0, "vision_overconf": 0, "tau_filter": 0,
        "divergence": 0, "ema_diff": 0, "consensus": 0,
        "availability": 0, "breaker": 0, "data_stale": 0,
        "confidence_gate": 0, "regime_window": 0, "slippage": 0,
        "trend_phase": 0, "phase_rhythm": 0, "time_decay": 0, "regime_weight": 0,
        # v3.655: SYS-025~028 量价增强
        "trend_health": 0, "donchian_vol": 0, "vp_l2": 0, "vp_stop_anchor": 0,
        # v3.660: N字门控实际拦截 + Vision N字冲突
        "n_gate_block_active": 0, "vision_n_conflict": 0,
        # KEY-003: 价值分析验证流水
        "key003_validate": 0, "key003_fail": 0,
        # KEY-004 T06: 治理+N字转换
        "key004_governance": 0, "key004_n_transition": 0, "key004_n_eval": 0,
    }

    # 同时读server.log和scan engine日志 (含.1 rotate回退)
    log_files = [
        os.path.join("logs", "server.log"),
        os.path.join("logs", "server.log.1"),
        os.path.join("logs", "price_scan_engine.log"),
        os.path.join("logs", "price_scan_engine.log.1"),
    ]
    tail = []
    for log_path in log_files:
        ...
        try:
            with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                lines = f.readlines()
                tail.extend(lines[-2000:] if len(lines) > 2000 else lines)
        except Exception:
            continue
    if not tail:
        return counts

    # 编译正则 (与log_analyzer_v3.py一致)
    patterns = {
        "rhythm_crypto": _re.compile(r'节奏过滤.*(?:BTC|ETH|SOL|ZEC|DOGE|XRP|ADA|AVAX)', _re.IGNORECASE),
        "vision_overconf": _re.compile(r'Vision过度自信降权|vision_overconfidence_penalty'),
        "tau_filter": _re.compile(r'τ过滤|tau_filter|噪声样本排除'),
        "divergence": _re.compile(r'view_divergence|视角分歧'),
        "ema_diff": _re.compile(r'EMA[58].*观察|EMA窗口分化'),
        "consensus": _re.compile(r'共识度评分|共识度=|consensus_score'),
        "availability": _re.compile(r'可用度|availability_score'),
        "breaker": _re.compile(r'熔断|breaker.*trigger|portfolio_breaker'),
        "data_stale": _re.compile(r'数据陈旧|staleness'),
        "confidence_gate": _re.compile(r'置信度门控|CONFIDENCE_GATE|confidence_gate'),
        "regime_window": _re.compile(r'Regime窗口|regime.*lookback'),
        "slippage": _re.compile(r'滑点|slippage'),
        "trend_phase": _re.compile(r'趋势阶段|阶段风险\[观察\]|trend_phase'),
        "phase_rhythm": _re.compile(r'阶段风险\[观察\]'),
        "time_decay": _re.compile(r'时间衰减\[观察\]|时间衰减|holding.*decay'),
        "regime_weight": _re.compile(r'Regime权重|CONSENSUS_ADAPTIVE|regime.*weight'),
        # v3.655: SYS-025~028
        "trend_health": _re.compile(r'趋势健康度\[观察\]|趋势健康度'),
        "donchian_vol": _re.compile(r'Donchian量价\[观察\]|Donchian量价'),
        "vp_l2": _re.compile(r'VP\+L2\[观察\]'),
        "vp_stop_anchor": _re.compile(r'VP止损锚定\[观察\]'),
        # v3.660: N字门控
        "n_gate_block_active": _re.compile(r'\[N_GATE拦截\]'),
        "vision_n_conflict": _re.compile(r'\[VISION_OVERRIDE\].*N字冲突'),
        "key003_validate": _re.compile(r'\[KEY-003\]\[VALIDATION\]\[MODULE\]\s+T\d{2}\s+(?:PASS|FAIL)'),
        "key003_fail": _re.compile(r'\[KEY-003\]\[VALIDATION\]\[FAIL\]'),
        # KEY-004 T06
        "key004_governance": _re.compile(r'\[KEY-004\]\[GOVERNANCE\]'),
        "key004_n_transition": _re.compile(r'\[KEY-004\]\[N_TRANSITION\]'),
        "key004_n_eval": _re.compile(r'\[KEY-004\]\[N_EVAL\]'),
    }
...
def _print_live_row(item, live_data, W_id, W_title, W_phase, W_verdict, W_detail):
    """打印单行改善项实时状态"""
    item_id = item.get("id", "")
    title = _imp_truncate(item.get("title", ""), W_title)

    # 阶段
    phase_str = ""
    if item.get("phase"):
        phase_str = _imp_truncate(item["phase"].replace("Phase 1 — ", "P1:").replace("Phase 1", "P1"), W_phase)

    # 实时数据
    ld = live_data.get(item_id, {"verdict": "NO_DATA", "detail": "未配置"})
    verdict = ld["verdict"]
    detail = _imp_truncate(ld.get("detail", ""), W_detail)

    # 颜色
    if verdict == "ACTIVE":
        vcol = C_G
    elif verdict in ("DEPLOYED", "COLLECTING"):
        vcol = C_C
    elif verdict == "PENDING":
        vcol = C_Y
    else:
        vcol = C_R
    pcol = C_R if item.get("priority") == "P0" else (C_Y if item.get("priority") == "P1" else C_W)

    row = "| " + pcol + pad_to_width(item_id, W_id) + C_0 + " | "
    row += pad_to_width(title, W_title) + " | "
    row += pad_to_width(phase_str, W_phase) + " | "
    row += vcol + pad_to_width(verdict, W_verdict) + C_0 + " | "
    row += pad_to_width(detail, W_detail) + " |"
    print(row)
...
def _generate_vision_daily_report_data(regime_data):
    """v3.640: 生成4方对比每日报告数据 (缠论规则/Vision/x4道氏/x4缠论)"""
    if not regime_data:
        return None
    ...
    # v3.640: 读取dual_track数据，生成4方准确率对比 (缠论规则/Vision/x4道氏/x4缠论)
    ...
    if dual_track and dual_track.get("stats"):
        ...
        for sym, s in dt_stats.items():
            ...
            x4_dow_total = s.get("x4_dow_total", 0)
            x4_dow_acc = s.get("x4_dow_correct", 0) / x4_dow_total if x4_dow_total >= 5 else None
            x4_chan_total = s.get("x4_chan_total", 0)
            x4_chan_acc = s.get("x4_chan_correct", 0) / x4_chan_total if x4_chan_total >= 5 else None
            ...
            if x4_chan_acc is not None and x4_dow_acc is not None and x4_chan_acc > x4_dow_acc:
                x4_using = "缠论"
            using_str = f"{l1_using}+x4{x4_using}"
            ...
            report["accuracy_comparison"][sym] = {
                ...
                "x4_dow": round(x4_dow_acc, 3) if x4_dow_acc is not None else None,
                "x4_chan": round(x4_chan_acc, 3) if x4_chan_acc is not None else None,
                "l1_using": using_str,
                "samples": rule_total,
            }
...
def display(state, validation_data, calibration_data, xgboost_stats, cnn_stats, regime_data, dual_track_data):
    ...
    print("\n " + "=" * 120)
    print(" AIPRO 决策层监控 v3.640 | " + now + " | 缠论趋势 + L1 Wyckoff + L2五大类评分 + Vision视觉分析")
    print(" " + "=" * 120)
    ...
    # ============================================================
    # 核心面板 6: 准确率对比表 (v3.570, 已禁用 — 数据依赖regime_validation未运行)
    # ============================================================
    ...
    # ============================================================
    # 核心面板 8: 三轨改善追踪 (v3.650)
    # ============================================================
    print_improvement_tracker()

    # ============================================================
    # 核心面板 9: TESTING验证面板 (v3.653)
    # ============================================================
    print_testing_verification()

    # ============================================================
    # 核心面板 10: DualPath 性能监控 (GCC-0146)
    # ============================================================
    print_dualpath_status()
    ...
    if verbose:
        ...
        print_orderflow_panel()  # v3.653: OrderFlow量价监控
        print_system_improvements_panel()  # v3.654: 系统改善监控
    ...
    print()
    print(" v3.640 | 刷新: %d秒 | MONITOR_VERBOSE=1 显示详细面板 | Ctrl+C退出" % REFRESH_INTERVAL)


def main():
    print(" 启动 AIPRO 决策监控 v3.640 (缠论趋势 + L1 Wyckoff定位 + L2五大类评分 + Vision视觉分析)...")
                ...
                if ny_now.hour == 8 and ny_now.minute < 1 and not vision_report_generated:
                    ...
                    if report_data:
                        ...
                        with open(log_file, "w", encoding="utf-8") as f:
                            ...
                            # v3.640: 4方准确率对比表
                            acc_comp_log = report_data.get("accuracy_comparison", {})
                            if acc_comp_log:
                                f.write("\n4方准确率对比 (谁准用谁):\n")
                                f.write(f"  {'品种':<10} {'缠论':>6} {'Vision':>7} {'x4道氏':>7} {'x4缠论':>7} {'采用':>12}\n")
                                for sym, acc in acc_comp_log.items():
                                    r = f"{acc['rule']*100:.1f}%" if acc.get('rule') is not None else "---"
                                    v = f"{acc['vision']*100:.1f}%" if acc.get('vision') is not None else "---"
                                    x4d = f"{acc['x4_dow']*100:.1f}%" if acc.get('x4_dow') is not None else "---"
                                    x4c = f"{acc['x4_chan']*100:.1f}%" if acc.get('x4_chan') is not None else "---"
                                    u = acc.get("l1_using", "缠论")
                                    f.write(f"  {sym:<10} {r:>6} {v:>7} {x4d:>7} {x4c:>7} {u:>12}\n")
                            disagreements = report_data.get("disagreements", [])
                            if disagreements:
                                f.write("\n分歧样本:\n")
                        ...
                        # v3.640: 发送4方对比报告邮件 (缠论规则/Vision/x4道氏/x4缠论)
                        try:
                            ...
                            if acc_comp:
                                email_lines = [
                                    ...
                                    "=" * 60,
                                    f"{'品种':<10} {'缠论':>6} {'Vision':>7} {'x4道氏':>7} {'x4缠论':>7} {'采用':>8}",
                                    "-" * 60,
                                ]
                                for sym, acc in acc_comp.items():
                                    ...
                                    x4c = f"{acc['x4_chan']*100:.1f}%" if acc.get('x4_chan') is not None else "---"
                                    u = acc.get("l1_using", "缠论")
                                    email_lines.append(f"{sym:<10} {r:>6} {v:>7} {x4d:>7} {x4c:>7} {u:>8}")
                                email_body = "\n".join(email_lines)
                                import requests
                                try:
                                    requests.post(
                                    ...
                                    print("[v3.640] 4方对比报告邮件已发送")
                                except Exception:
                                    print("[v3.640] 邮件API不可达，跳过")
                        except Exception as email_e:
                            print(f"[v3.640] 邮件发送异常: {email_e}")
